...
router.get("/vnpay_return", function (req, res, next) {
  ...
  if (secureHash === signed) {
    ...
    console.log("Debug_here orderId: ", orderId);
    ...
    Order.findByIdAndUpdate(
      ...
      function (err, doc) {
        if (err) {
          return res.redirect("http://localhost:4001/order");
        } else {
          return res.redirect(
            `http://localhost:4001/order/${orderId}?message=Thanh+to%C3%A1n+th%C3%A0nh+c%C3%B4ng&checkoutStatus=true`
          );
        }
      }
    );
    ...
    return res.redirect(
      `http://localhost:4001/order/${orderId}?message=Thanh+to%C3%A1n+th%E1%BA%A5t+b%E1%BA%A1i&checkoutStatus=false`
    );
  }
});
